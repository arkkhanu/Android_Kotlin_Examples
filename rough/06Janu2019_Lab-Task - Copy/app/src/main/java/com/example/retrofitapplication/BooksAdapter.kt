...
class BooksAdapter(val data:ArrayList<VolumesResponse.VolumeItem>) : RecyclerView.Adapter<BooksAdapter.BookViewHolder>(){
    override fun onCreateViewHolder(p0: ViewGroup, p1: Int): BookViewHolder =
        BookViewHolder(LayoutInflater.from(p0.context).inflate(R.layout.book_item,p0,false))
    ...
    class BookViewHolder(v: View) : RecyclerView.ViewHolder(v) {
        ...
        val bookTitle = itemView.findViewById<TextView>(R.id.bookTitleTv)
        val bookAuther= itemView.findViewById<TextView>(R.id.bookAuthorsTv)

        fun bindBook(book:VolumesResponse.VolumeItem)
        {
            bookTitle.text = book.volumeInfo?.title
            bookAuther.text = book.volumeInfo?.authors?.joinToString()
